...
# 修改后的标准卷积，允许传递 stride(s)、padding(p) 等参数
class Conv(nn.Module):
    # Standard convolution with optional stride and padding
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, bias=False):
        """
        :param c1: in_channels
        :param c2: out_channels
        :param k: kernel_size
        :param s: stride
        :param p: padding (若不指定则默认自动计算为 (k-1)//2 )
        :param g: groups
        :param bias: bool
        """
        ...
        if p is None:
            p = (k - 1) // 2
        self.conv = nn.Conv2d(c1, c2, kernel_size=k, stride=s, padding=p, groups=g, bias=bias)
...
class RFAConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride=1):
        ...
        # 这里使用了 Conv，传入 s=kernel_size、p=0
        # 修正后：Conv 类已支持 stride(s) 和 padding(p) 参数
    ...
    def forward(self, x):
        ...
        conv_data = rearrange(weighted_data, 'b c (n1 n2) h w -> b c (h n1) (w n2)',
                              n1=self.kernel_size, n2=self.kernel_size)  # b c h*k w*k
...
class RFCAConv(nn.Module):
    ...
    def forward(self, x):
        ...
        h2, w2 = generate_feature.shape[2:]
        x_h, x_w = torch.split(y, [h2, w2], dim=2)
